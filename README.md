...
│   ├── photo-albums.json       ← Shared photo collections (referenced by photo_set)
│   └── leg-XX-….json           ← Transcribed stops & entries (source of truth)
├── images/
│   ├── scans/                  ← Cropped logbook images, one per entry (by date)
│   └── photos/
│       ├── intake/             ← Drop new scanned prints here before sorting
│       ├── albums/             ← Shared location albums (by region)
│       ├── web/                ← Web-sized copies (optional; resize script later)
│       └── thumbs/             ← Thumbnails (optional; resize script later)
...
| Trip photos for the site | `images/photos/albums/` (and optionally `web/` + `thumbs/`) | Region folders, e.g. `albums/bahamas/bahamas_001.jpg` |
...
| `photo_album` | URL to external full-res album (Amazon Photos, etc.) — overrides album default when set |
| `photo_set` | Key into `data/photo-albums.json` for a **shared location album** (multiple stops can point to the same collection) |
| `photos` | Per-stop inline photos — usually leave empty when using `photo_set` |
...
## Workflow: trip photos

Trip photos for the early voyage are **scanned prints**, grouped by region rather than by individual anchorage. The site uses **shared albums** so the same 5 Bahamas photos are not duplicated across seven stops.

### Storage layout

```
images/photos/albums/
  bahamas/bahamas_001.jpg
  jamaica/jamaica_001.jpg
  san-blas/sanblas_islands_001.jpg
  …
data/photo-albums.json    ← catalog of albums and file paths
```

Drop new scanned prints in `images/photos/intake/`, sort into the right `albums/<key>/` folder, then add entries to `photo-albums.json`.

### Shared albums (`photo-albums.json` + `photo_set`)

**`data/photo-albums.json`** holds one entry per region/collection:
...
```json
{
  "albums": {
    "bahamas": {
      ...
      "amazon_album": null,
      "photos": [
        { "file": "images/photos/albums/bahamas/bahamas_001.jpg", "caption": "Bahamas — photo 1" }
      ]
    }
  }
}
...
On each stop that shares the collection, set `photo_set` to the album key (keep `photos: []`):
...
Multiple stops (e.g. Gun Cay through George Town) can all use `"photo_set": "bahamas"`. The **Photo Galleries** button in the header lists each album once; the map shows a gold camera badge on stops that have photos.

### Per-stop or external albums

For a stop-specific gallery or Amazon link only:

```json
"photo_album": "https://www.amazon.com/photos/share/…",
"photos": [
  {
    "file": "images/photos/albums/galapagos/galapagos_001.jpg",
    "thumb": "images/photos/thumbs/galapagos_001.jpg",
    "caption": "Sunset at anchor"
  }
]
```

When `photos` or `photo_set` is set, a **View photo gallery** button appears on that stop. Thumbnails open in an on-site lightbox (not a new browser tab). Set `amazon_album` in `photo-albums.json` or `photo_album` on a stop for full-resolution external viewing.

### Future: digital photos + resize script

For timestamped digital photos later in the voyage, optional `web/` and `thumbs/` copies can be generated by `scripts/resize-photos.py` (not yet added). Full-res originals stay in `images/originals/` (gitignored) or on Amazon Photos.

### Intake workflow

1. Drop scans in `images/photos/intake/`
2. Move into `images/photos/albums/<album-key>/`
3. Add paths to `data/photo-albums.json`
4. Set `photo_set` on the relevant stops in the leg JSON
5. Preview with `python -m http.server 8000`
